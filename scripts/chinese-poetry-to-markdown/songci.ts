...
export function syncSongCi() {
    try {
        // 读取宋词目录
        const files = readdirSync('chinese-poetry-master/宋词')
        const targetFiles = files.filter(file => file.startsWith('ci.song') || file.startsWith('宋词三百首'))

        let totalPoems = 0
        let successCount = 0
        let skippedCount = 0

        console.log(`开始处理全宋词，共 ${targetFiles.length} 个文件`)
        ...
        for (let i = 0; i < targetFiles.length; i++) {
            ...
            try {
                const poems = readJsonSync(`chinese-poetry-master/宋词/${file}`)
        ...
        console.error('宋词同步过程中发生错误:', error)
        throw error
    }
}
